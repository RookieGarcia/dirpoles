...
if (isset($id_orientacion)) {
    $beneficiario = $datos['nombre_beneficiario']. ' '. $datos['apellido_beneficiario'];
    $cedula = $datos['cedula'];
    $fecha = $datos['fecha_creacion'];
}
...
if(isset($id_psicologia)){
    $beneficiario = $datos['nombre_beneficiario']. ' '. $datos['apellido_beneficiario'];
    $cedula = $datos['cedula_beneficiario'];
    $fecha = $datos['fecha_psicologia'];
}
...
$pdf->Image('pdf/constancia/constancia.png', 0, 0, 210, 297); // Ajusta las coordenadas y dimensiones según la imagen y tamaño de página (A4 en mm)
